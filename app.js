...
let state=null, editorChart=null, editorPage=0, noteType='tap', editorDirty=false, loading=false, pendingAudioSong=null;
...
function applyAppearance() {
  ...
  if(theme.avatar){
    ...
    setImage('.game-character',theme.card,'');
...
function renderDetail() {
  const song=selected;if(!song)return;const saved=charts[song.id];
  ...
  $('#songDetail').innerHTML=`<div class="detail-topline"><span>YOUR NEXT LIVE</span><button id="favoriteSong" class="icon-button ${favorites.has(song.id)?'heart':''}" aria-label="${favorites.has(song.id)?'Remove from':'Add to'} favorites">${favorites.has(song.id)?'♥':'♡'}</button></div><div class="detail-cover">✦${cover(song)?`<img src="${safe(cover(song))}" alt="${safe(song.title)} cover">`:''}</div><h2>${safe(song.title)}</h2><p class="japanese">${safe(song.japanese)}</p><span class="unit-tag">${safe(song.group)}</span><div class="details-meta"><div><small>ARTIST</small><strong>${safe(song.artist)}</strong></div><div><small>TEMPO</small><strong>${saved?.bpm||song.bpm} ${song.estimated&&!saved?'est. ':''}BPM</strong></div><div><small>LENGTH</small><strong>${time(bufferCache.get(song.id)?.duration||song.duration)}</strong></div></div><p class="small-heading">CHOOSE YOUR DIFFICULTY</p><div class="difficulties">${[['easy','EASY',6],['normal','NORMAL',12],['hard','HARD',19]].map(([id,label,n])=>`<button class="difficulty ${difficulty===id?'active':''}" data-diff="${id}">${label}<b>${n}</b></button>`).join('')}</div><p class="audio-note">${ready(song)?'● Audio ready':'○ Online audio · loads when you play'}<br>${saved?'Your saved chart is selected. Try the new demo below for fresh patterns.':'Song-specific practice chart · taps, holds & upward flicks.'}${song.estimated&&!saved?' Tempo is an estimate; adjust it in Chart Studio.':''}</p><button id="playSong" class="primary full" ${loading?'disabled':''}>${loading?'Preparing audio…':'▶ &nbsp; Play '+(saved?'my chart':'demo')}</button><div class="detail-controls"><button id="openEditor" class="secondary">＋ Chart Studio</button><button id="attachAudio" class="secondary">↑ Replace audio</button></div>${saved?'<button id="playDemo" class="text-button full">Try the new demo arrangement</button>':''}<div class="key-guide"><div>${settings.keys.map(key=>`<kbd>${safe(keyLabel(key))}</kbd>`).join('')}</div><p>↑ Flick: ${settings.flickMode==='tap'?'tap assist':safe(keyLabel(settings.flickKey))+' + lane'} · or swipe up</p><button id="editBindings" class="text-button">Customize controls →</button><p class="help">${records[song.id]?'Personal best · '+Number(records[song.id]).toLocaleString():'Hit the line. Follow the rhythm.'}</p></div>`;
...
async function startGame(chart,fromEditor=false) {
  ...
  try{
    ...
    state={song,chart,notes,buffer,readyAt,started:readyAt-startOffset,position:startOffset,countdown:3,paused:false,finished:false,score:0,combo:0,maxCombo:0,life:1000,counts:{PERFECT:0,GREAT:0,GOOD:0,BAD:0,MISS:0},flashes:Array(6).fill(-1000),particles:[],lastJudge:0,fromEditor,custom};
    setView('game');$('#gameOverlay').classList.add('hidden');$('#playingTitle').textContent=song.title;$('#playingInfo').textContent=`${chart.bpm} BPM · ${notes.length} notes · ${custom?'CUSTOM':'REMIX DEMO'}`;
    $('#gameMode').textContent=fromEditor?'CHART TEST':settings.practice?'PRACTICE · NO FAIL':'CHALLENGE';$('#judgment').textContent='';applyAppearance();playBuffer(buffer,readyAt,startOffset);updateHUD();$('#toast').classList.remove('show');
...
function judge(note,result) {
  ...
  else{state.combo++;state.maxCombo=Math.max(state.maxCombo,state.combo);state.life=Math.min(1000,state.life+5);state.flashes[note.lane]=performance.now();successfulSound(note,result,['hold','slide'].includes(note.type)?'release':'note');}
...
function hit(lane,isFlick=false) {
  ...
  if(['hold','slide'].includes(note.type)&&result!=='BAD'){
    note.status='holding';note.headJudgment=result;note.nextCheckpoint=1;successfulSound(note,result);
  }else judge(note,result);
}
function flick(lane) {if(!activePlay())return;const t=gameTime(),note=state.notes.find(n=>n.lane===lane&&n.type==='flick'&&n.status==='pending'&&Math.abs(t-n.t)<=.18);if(note)judge(note,judgmentFor(t-note.t));}
...
function release() {
  ...
  const t=gameTime();
  for(const note of state.notes)if(note.status==='holding'&&!isHeld(note,t))judge(note,t>=note.end-.1?note.headJudgment:'MISS');
}
function pauseGame() {
  if(!state||state.finished||view!=='game')return;if(state.paused){resumeGame();return;}state.position=gameTime();state.paused=true;stopSource();clearInputs();
  ...
  $('#gameOverlay').classList.remove('hidden');$('#resume').onclick=resumeGame;$('#restart').onclick=()=>startGame(state.chart,state.fromEditor);$('#pauseSettings').onclick=()=>showSettings('controls');$('#leave').onclick=leaveGame;
}
async function resumeGame() {if(!state?.paused||$('#settingsDialog').open)return;await audioInit();state.readyAt=audioContext.currentTime+1.2;state.started=state.readyAt-state.position;state.paused=false;state.resumeGrace=state.readyAt+.15;playBuffer(state.buffer,state.readyAt,state.position);$('#gameOverlay').classList.add('hidden');}
function finishGame(failed=false) {
  if(!state||state.finished)return;state.position=gameTime();state.finished=true;stopSource();clearInputs();const score=Math.round(state.score);
  if(!failed&&score>(records[state.song.id]||0)){records[state.song.id]=score;write('six-records',records);}
  const total=Object.values(state.counts).reduce((a,b)=>a+b,0),accuracy=total?((state.counts.PERFECT+state.counts.GREAT*.8+state.counts.GOOD*.5+state.counts.BAD*.2)/total*100).toFixed(1):'0.0';
  $('#gameOverlay').innerHTML=`<div class="overlay-card"><p class="eyebrow">${failed?'KEEP PRACTICING':'LIVE COMPLETE'}</p><h2>${failed?'One more try?':state.counts.MISS+state.counts.BAD===0?'Full combo!':'Nice session.'}</h2><p>${safe(state.song.title)} · ${accuracy}% accuracy</p><div class="result-score">${score.toLocaleString()}</div><div class="result-grid"><span>MAX COMBO<b>${state.maxCombo}</b></span><span>PERFECT<b>${state.counts.PERFECT}</b></span><span>MISSES<b>${state.counts.MISS}</b></span><span>GREAT<b>${state.counts.GREAT}</b></span><span>GOOD<b>${state.counts.GOOD}</b></span><span>BAD<b>${state.counts.BAD}</b></span></div><button id="retry" class="primary full">↻ Play again</button><button id="resultBack" class="glass-button full">${state.fromEditor?'Back to Chart Studio':'Back to library'}</button></div>`;
  $('#gameOverlay').classList.remove('hidden');$('#retry').onclick=()=>startGame(state.chart,state.fromEditor);$('#resultBack').onclick=leaveGame;
}
function leaveGame() {stopSource();clearInputs();const back=state?.fromEditor;state=null;if(back){setView('editor');drawWave();}else{setView('library');renderLibrary();renderDetail();}}
...
function frame(now) {
  ...
  const t=gameTime();state.countdown=Math.max(0,Math.ceil(state.readyAt-audioContext.currentTime));
  if(activePlay()){
    for(const note of state.notes){
      if(note.status==='pending'&&t>note.t+.18)judge(note,'MISS');
        ...
        else if(t>=note.end-.025)judge(note,note.headJudgment);
...
canvas.addEventListener('pointerdown',event=>{
  ...
  canvas.setPointerCapture(event.pointerId);pointers.set(event.pointerId,{lane,y:event.clientY,last:performance.now(),distance:0,used:false});refreshPressed();hit(lane);
});
...
let dbPromise;
function db() {return dbPromise??=new Promise((resolve,reject)=>{const request=indexedDB.open('sekai-six-audio',1);request.onupgradeneeded=()=>request.result.createObjectStore('songs',{keyPath:'id'});request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error);});}
