...
function finishGame(failed=false) {
  ...
  const theme=getTheme(settings.theme),resultArt=theme.resultArt,celebrate=!failed&&(isNewBest||summary.badge==='ALL PERFECT'||summary.badge==='FULL COMBO');
  const art=resultArt?`<img class="results-card-art" src="${safe(resultArt.image)}" alt="${safe(resultArt.title)} — ${safe(theme.name)}" style="object-position:${safe(resultArt.position)}"><div class="results-art-unit">${safe(theme.unit)}</div><div class="results-card-caption"><span>${safe(theme.name)}</span><strong>${safe(resultArt.title)}</strong></div>`:'';
  const reaction=celebrate&&resultArt?`<img class="results-reaction" src="${safe(resultArt.sticker)}" alt="${safe(theme.short)}: ${safe(resultArt.reaction)}">`:'';
  ...
  $('#gameOverlay').innerHTML=`<section class="results-screen" data-art="${Boolean(resultArt)}" aria-labelledby="resultsTitle">
